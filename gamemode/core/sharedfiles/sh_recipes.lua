...
function Player:CanMake(strRecipe)
	...
	for strMaster, intLevel in pairs(tblRecipeTable.RequiredMasters) do 
		...
		if self:GetMasterLevel(strMaster) < intLevel then 
		
			return false 
		end 
	end
	
	for strItem, intAmount in pairs(tblRecipeTable.Ingredients) do 
	...
	if tblRecipeTable.MakeTime then 
		...
		if CurTime() < (self.NextCraft or 0) then 
		
			return false
		end 
	end
	
	if !self:HasRoomFor(tblRecipeTable.Products, -self:TotalWeightOf(tblRecipeTable.Ingredients)) then 
		return false 
	end
